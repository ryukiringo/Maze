...
class ViewController: UIViewController {
    ...
    func startAccelerometer() {
        let handler: CMAccelerometerHandler = {(CMAccelerometerData: CMAccelerometerData?, error: Error?) -> Void in
            ...
            if self.goalView.frame.intersects(self.playerView.frame) {
                self.gameCheck(result: "Game Clear", message: "ゴールにつきました")
                return
            }
    ...
    func gameCheck(result: String, message: String) {
        ...
        let gameCheckAlert: UIAlertController = UIAlertController(title: title, message: message, preferredStyle: .alert)
